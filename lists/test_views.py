...
class DisplayListCase(TestCase):
    ...
    def test_http_response(self) -> None:
        existing_list_response = self.client.get(
            reverse("todo", args=[self.test_list.pk])
        )
...
class ToggleItemCase(TestCase):
    def setUp(self) -> None:
        ...
        self.other_user = User.objects.create_user(
            username="some_other_guy", password="another_password"
        )
        self.client.login(username="test_user", password="test_password")
        self.test_list = Todo.objects.create(title="test_list", owner=self.user)
        self.other_guys_list = Todo.objects.create(
            title="I dunno what all this testing is about", owner=self.other_user
        )
        self.our_item = TodoItem.objects.create(
            name="This Item is Checked", completed=False
        )
        self.our_item.list.add(self.test_list)
        self.other_guys_item = TodoItem.objects.create(
            name="This Item is Unchecked", completed=False
        )
        self.other_guys_item.list.add(self.other_guys_list)

    def test_permissions(self) -> None:
        random_item_completed_before = self.other_guys_item.completed
        checking_random_item = self.client.get(
            reverse(
                "toggle-item",
                kwargs={
        ...
        self.assertEqual(
            random_item_completed_before,
            TodoItem.objects.get(pk=self.other_guys_item.pk).completed,
        )

    def test_checking_and_unchecking(self) -> None:
        our_item_completed_before = self.our_item.completed
        checking_our_item = self.client.get(
            reverse(
                "toggle-item",
                kwargs={
        ...
        self.assertNotEqual(
            TodoItem.objects.get(pk=self.our_item.pk).completed,
            our_item_completed_before,
        )
        unchecking_our_item = self.client.get(
            reverse(
                "toggle-item",
                kwargs={
                    "item": self.our_item.pk,
...
class ToggleListOnItemCase(TestCase):
    def setUp(self) -> None:
        ...
        self.client.login(username="test_user", password="test_password")
        self.test_list = Todo.objects.create(title="test_list", owner=self.user)
        self.test_other_list = Todo.objects.create(
            title="test_other_list", owner=self.user
        )
        self.test_item = TodoItem.objects.create(name="Test Item")
        self.test_item.list.add(self.test_list)
...
class ToggleStarredCase(TestCase):
    ...
    def test_star_list(self):
        response = self.client.get(
            reverse("toggle-starred", kwargs={"todo_pk": self.test_list.pk})
        )
...
class ListEditCase(TestCase):
    ...
    def test_http_response(self) -> None:
        ...
        response_edit_list = self.client.get(
            reverse("list-edit", kwargs={"list_pk": self.test_list.pk})
        )
        self.assertEqual(response_edit_list.status_code, 200)

    def test_new_list(self) -> None:
        response_new_list = self.client.post(
            reverse("list-edit"),
            {
        ...
        self.assertIsNotNone(
            Todo.objects.filter(title="a completely new list!").first()
        )
        new_list = Todo.objects.get(title="a completely new list!")
        response_new_sublist = self.client.post(
            reverse("list-edit"),
            {
        ...
        self.assertIsNotNone(
            Todo.objects.filter(title="a completely new sublist!").first()
        )
    ...
    def test_edit_existing_list(self) -> None:
        ...
        response_edit_toplevel = self.client.post(
            reverse("list-edit", args=[new_list.pk]),
            {
                "owner": self.user.pk,
        ...
        self.assertEqual(
            Todo.objects.get(pk=new_list.pk).title, "a slightly different toplevel list"
        )
        new_sublist = Todo.objects.create(title="a sublist", owner=self.user)
        response_edit_sublist = self.client.post(
            reverse("list-edit", args=[new_sublist.pk]),
            {
                "owner": self.user.pk,
...
class ItemDetailsCase(TestCase):
    ...
    def test_http_response(self) -> None:
        response_get_form = self.client.get(
            reverse("item-details", args=[self.test_item.pk])
        )
        self.assertEqual(response_get_form.status_code, 200)

    def test_post_item_changes(self) -> None:
        response_post_changes = self.client.post(
            reverse("item-details", args=[self.test_item.pk]),
            {
        ...
        self.assertEqual(
            TodoItem.objects.get(pk=self.test_item.pk).due_date,
